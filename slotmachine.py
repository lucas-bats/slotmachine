...
def get_number_of_lines():
    while True:
        lines = input(
            "Digite o número de linhas para apostar (1-" + str(MAX_LINES) + "): ")
        if lines.isdigit():
            lines = int(lines)
    ...
    while True:
        ...
        if amount.isdigit():
            ...
            if MIN_BET <= amount <= MAX_BET:
                ...
                print(f"O valor deve estar entre ${MIN_BET} e ${MAX_BET}.")
        else:
            print("Por favor, insira um número.")
...
def spin(balance):
    ...
    while True:
        ...
        if total_bet > balance:
            print(
                f"Você não tem dinheiro suficiente para apostar esse valor, seu saldo atual é: ${balance}")
        else:
            break
